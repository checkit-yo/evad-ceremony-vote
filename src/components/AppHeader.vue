...
<template>
  <header class="fixed top-0 left-0 right-0 z-50 transition-all duration-500" :class="[
      ...
      <div class="flex items-center justify-between h-20">
        ...
        <nav class="hidden lg:flex items-center gap-10">
          ...
          <div class="relative group">
            <button
              class="font-title text-xs uppercase tracking-widest text-cream-400 hover:text-gold transition-colors duration-500 flex items-center gap-2">
              Catégories
              <svg class="w-3 h-3 transition-transform duration-500 group-hover:rotate-180" fill="none"
            ...
            <!-- Dropdown -->
            <div
              class="absolute top-full left-0 mt-4 w-64 bg-burgundy border border-cream-300/20 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
              <div class="py-2">
        ...
        <!-- CTA Button (Desktop) -->
        <NuxtLink to="/#categories" class="hidden lg:inline-flex btn btn-gold text-xs">
          Voter
        </NuxtLink>

        <!-- Mobile Menu Button -->
        <button class="lg:hidden p-2 text-cream-300 hover:text-gold transition-colors" @click="emit('toggleMenu')"
    ...
    <Transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 -translate-y-4"
      ...
      <div v-if="isMenuOpen"
        class="lg:hidden bg-burgundy-950 backdrop-blur-md border-t border-gold/30 shadow-xl shadow-burgundy-950/50">
        <nav class="container mx-auto px-4 py-6 space-y-4">
          ...
          <div class="border-t border-cream-400/10 pt-4">
            <NuxtLink to="/#categories" class="btn btn-gold w-full text-xs mb-3" @click="emit('closeMenu')">
              Voter maintenant
            </NuxtLink>
            <a href="https://www.helloasso.com/associations/evad-creation/evenements/evad-ceremony" target="_blank"
              rel="noopener noreferrer"
              class="block w-full text-center font-title text-xs uppercase tracking-widest text-cream-400 hover:text-gold transition-colors py-2 border border-cream-400/20 hover:border-gold"
              @click="emit('closeMenu')">
              Billetterie
            </a>
          </div>
